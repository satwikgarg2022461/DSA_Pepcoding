...
// import isTreeBinarySearchTree.BSTPair;
...
public class isBalanced {
    ...
    public static BalancePair isBalance(Node nd){
        if(nd  == null){
        ...
        BalancePair left = isBalance(nd.left);
        BalancePair right = isBalance(nd.right);

        BalancePair mp = new BalancePair();
        mp.ht = Math.max(left.ht, right.ht) + 1;
        // mp.rh = Math.max(right.lh, right.rh) + 1;
        mp.isBalanced = left.isBalanced && right.isBalanced && Math.abs(left.ht - right.ht)<1;
    ...
    public static void main(String[] args) throws IOException{
        ...
        // display(root);
        BalancePair ans = isBalance(root);
