...
public class allPathDFS {
    ...
    public static void path(ArrayList<Edge>[] graph, int src,int des, boolean[] visited, String path ){
        if(src == des){
        ...
        for(Edge edge : graph[src]){
            if(visited[edge.des] == false){
                path(graph, edge.des, des, visited, path+edge.des);
            }
        }
        visited[src] = false;
    }
